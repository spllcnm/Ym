  ...
  return new Promise((resolve) => {
    ...
    $.post(url, async (err, resp, data) => {
      try {
        const result = JSON.parse(data)
        if (result.code == 200) {
          $.log(`\n多点淘金app扫码验证:${result.data.invite_url}&dati=1`)
    ...
    $.post(url, async (err, resp, data) => {
      try {
        ...
        if (result.code == 200) {
          ...
          $.log(`\n多点淘金余额不足:【跳过提现】现有余额:【${result.data.money.balance}】`)
          }
...
//团队提现判断
function ddtjtxtd(timeout = 0) {
  return new Promise((resolve) => {

    let url = {
      url: 'https://api.gzswin.cn/spread/index',
      headers: JSON.parse(`{"Host":"api.gzswin.cn","Accept":"*/*","User-Agent":"Mozilla/5.0 (iPad; CPU OS 14_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.16(0x1800103f) NetType/WIFI Language/zh_CN","Content-Type":"application/json","Content-Length":"62"}`),
      body: `{"token":"${ddtjtoken}"}`
    }
    $.post(url, async (err, resp, data) => {
      try {
        const result = JSON.parse(data)
        if (result.code == 200) {
          ddtjtdtx = result.data.score_money
          if(yue >= 0.3){
            $.log(`\n吊毛~你的团队可提现余额: 【${result.data.score_money}】 根毛，去拔毛咯！`)
            $.log(`\n今日团队收益: 【${result.data.score_money_today}】 元`)
            $.log(`\n昨日团队收益: 【${result.data.score_money_yesterday}】 元`)
            await $.wait(200)
            await ddtjtx1()
          }else {
          
          $.log(`\n多点淘金团队余额不足:【跳过提现】现有余额：【${result.data.score_money}】`)
          $.log(`\n今日团队收益: 【${result.data.score_money_today}】 元`)
          $.log(`\n昨日团队收益: 【${result.data.score_money_yesterday}】 元`)
          }
...
//团队提现
function ddtjtx1(timeout = 0) {
  return new Promise((resolve) => {
    ddtjjm = Encrypt(`{"score_money":${ddtjtdtx},"time":"${ddtjtime}"}`)
    let url = {
      url: 'https://api.gzswin.cn/wxpay/score_money',
      headers: JSON.parse(`{"Host":"api.gzswin.cn","Accept":"*/*","User-Agent":"Mozilla/5.0 (iPad; CPU OS 14_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.16(0x1800103f) NetType/WIFI Language/zh_CN","Content-Type":"application/json","Content-Length":"62"}`),
      body: `{"data":"${ddtjjm}","token":"${ddtjtoken}"}`
    }
    $.post(url, async (err, resp, data) => {
