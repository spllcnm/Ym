...
https://api.yd.ihaoqu.com/?r=api2&apiAction=SignIn 接口链接。重写也是这个。。。
重新打开小程序会掉登录。。
...
function bhxcyck() {
  if ($request.url.indexOf("SignIn") > -1) {
    const bhxcytoken = JSON.stringify($request.body)
    if (bhxcytoken) $.setdata(bhxcytoken, `bhxcytoken${status}`)
    $.log(bhxcytoken)
    $.msg($.name, "", '渤海宣传员' + `${status}` + 'token获取成功！')
  }
}
//签到
